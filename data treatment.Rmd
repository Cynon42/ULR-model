...
triangle = data_steps %>%filter(DevMth>0)%>%
  group_by(SelYOA,DevMth,ReservingClass,Country)%>%
...
# get exposure data 
exposure_table = eur.dat.cul.mth%>%select(-(7:23))%>% group_by(ReservingClass,YOA,Country,DevMth)%>%summarise_all(last)%>% group_by(ReservingClass,YOA,Country)%>%select(-2,-3,-6)%>%summarise(NetPrem = sum(NetPrem))
...
stan_data_multisegment = triangle_inc_exposure%>%ungroup()%>%
  mutate(year = YOA-min(YOA)+1,
         exposure = NetPrem,
         segment = as.numeric(factor(as.numeric(factor(Country))&as.numeric(factor(ReservingClass))))
         )%>%
  group_by(segment,year)%>% arrange(DevMth)%>%
  ...
  rename(t = DevMth,
         ...
         N_increment = N,
         country=Country,reservingclass=ReservingClass,
         N = N_cum
         )%>%
  ...
  select(country,
         ...
         N_pos_increment,
         N_increment,
         N,Nprev)%>%mutate(year_segment = as.numeric(factor(year*segment)))
