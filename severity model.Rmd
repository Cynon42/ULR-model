...
#formula_sev <- bf(ACPC_increment  ~ exp(exp(a)*ult+b+7+log_diff_exp(lognormal_lcdf_mod(DevMth,log(60)-2.3*exp(sigmadev)-exp(mudev),exp(sigmadev))#,lognormal_lcdf_mod(DevMthprev,log(60)-2.3*exp(sigmadev)-exp(mudev),exp(sigmadev))))
#                        #, nlf(sigma ~ exp(sigmastd-(exp(lambda)-log(0.01)/60)*DevMth))# expon ccdf for sigma. >99% dev by t=60
#                        #, nlf(sigma ~ #exp(sigmastd-log(nzClm)+lognormal_lccdf_mod(DevMth,log(60)-2.3*exp(sigmadev)-exp(mudev),exp(sigmadev)+exp(lambda))) )
#                        , nlf(sigma ~ exp(sigmastd-log(nzClm)+lognormal_lccdf_mod(DevMth,2,1)) )
#                        , sigmastd ~ (1|id|ReservingClass)+(1|id2|year_segment)+t2(YOA,by=ReservingClass)
#                        #, lambda ~ (1|id|ReservingClass)+(1|id2|year_segment)+t2(YOA,by=ReservingClass)
#                       ,mudev ~ (1|id|ReservingClass)+(1|id2|year_segment)+t2(YOA,by=ReservingClass)
#                       ,sigmadev~ (1|id|ReservingClass)+(1|id2|year_segment)+t2(YOA,by=ReservingClass)
#                       ,ult~ (1|id|ReservingClass)+(1|id2|year_segment)+t2(YOA,by=ReservingClass)
#                       ,a+b ~ (1|id|ReservingClass)
#                       ,family = brmsfamily("gaussian",link = "identity",link_sigma = "identity")
#                       ,nl=TRUE)


formula_sev <- bf(ACPC_increment  ~ a*exp(-exp(b)*(DevMth-75)-3)+exp(ult+7)
                        , nlf(sigma ~ exp(c-exp(d)*(DevMth-75)-5-log(nzClm^0.5)) )
                       ,a ~ (1|id|ReservingClass)+(1|id2|year_segment)+t2(DevMth,by=year_segment)
                      ,b ~ (1|id|ReservingClass)
                       ,c+d ~ (1|id|ReservingClass)
                  ,ult ~ (poly(YOA,3)|id|ReservingClass)+(1|id2|year_segment)
                       ,family = brmsfamily("gaussian",link = "identity",link_sigma = "identity")
                       ,nl=TRUE
                       )
