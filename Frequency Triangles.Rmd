...
formula_def = bf(N_group_increment | vint(N,Nprev,N_pos_increment,modelflag) ~ lambdapos + log(exposure),
   ...
   nlf(mup~log(60)-2.3*sigmap-exp(mupos)),# Constrained mu for >99% development at t=60
   ...
   Upos ~ 1,
   mupos  + sigmapos ~ 1,
   nlf(tz~ t),#exp((log(t)-mup)/sigmap)),
   nlf(tprevz~ tprev),#exp((log(tprev)-mup)/sigmap)),
   ...
   nlf(lambdaneg ~ exp( logdifffunction(tz,tprevz,mun,sigman) - exp(Uneg) - logdifffunction(t,0.01,mup,sigmap)) ), # this function will be positive and never greater than 1
   Uneg + muneg  + sigmaneg ~ 1,
  family = freq_def,
  nl=TRUE)
...
for (i in unique(pred_increments$year)){
  ...
  N_plot[[k]] = pred_increments%>%
                ...
                stat_lineribbon() +
                scale_fill_brewer() +
                geom_point(data = pred_increments%>%filter(year ==(i)), aes(t, N))
  Npos_plot[[k]] = pred_increments%>%
                ...
                stat_lineribbon() +
                scale_fill_brewer() +
                geom_point(data = pred_increments%>%filter(year ==(i)), aes(t, N_pos_cum))
  Nneg_plot[[k]] = pred_increments%>%
                ...
                stat_lineribbon() +
                scale_fill_brewer() +
                geom_point(data = pred_increments%>%filter(year ==(i)), aes(t, N_pos_cum-N))
