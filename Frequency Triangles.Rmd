...
freq_def <- custom_family(
                    ...
                    type = "int",# lpmf vs lpdf
                    vars = c("vint1[n]", # N
                             "vint2[n]" # model flag where 1 = N_pos_increment
                             ) 
              )
```
...
```{r}
library(brms)
formula_def = bf(N_group_increment | vint(N,modelflag) ~ lambdapos + log(exposure),
   ...
   nlf(mun~10-2.3*sigman-exp(muneg+1)),# Constrained mu for >99% development at z=5
...
mypriors_def <- c(
  ...
  prior(normal(3, 1),  nlpar = "muneg"),
  prior(normal(-1, 1),  nlpar = "sigmaneg")
  #prior(normal(0, 1),class="Intercept",  dpar = "phineg")
  )
```
