...
real sharedpars_exposure_lpmf(int Nposincrement,
  ...
  // (Nest[2,2]-Nest[1,2])/Nest[1,1]) or (expected Nnegincrement)/(expected Nprev)
  return binomial_lpmf(Nnegincrement|Nprev,fmin(0.99,(Nest[2,2]-Nest[1,2])/Nest[1,1])) + poisson_log_lpmf(Nposincrement|log(exposure) - log(1e7)+ log(Upois) + log_diff_exp(lognormal_lccdf(tprev|log(60)-2.3*sigmapois-exp(mupois),sigmapois), lognormal_lccdf(t|log(60)-2.3*sigmapois-exp(mupois),sigmapois)));
...
library(brms)
library(dplyr)
model_triangles_binom = brm(
  formula_binom,
  prior = mypriors_binom,
  stanvars = stanvar(scode = stan_functions_binom, block = "functions"),
  data = stan_data_multisegment%>%filter(as.numeric(segment)<3), 
  backend = "cmdstan",
  threads = threading(12),
  control = list(adapt_delta = 0.99),
  max_treedepth=20,
