...
formula_exposure <- bf(N_pos_increment | vint(Nprev,N)+vreal(t,tprev,exposure) ~ (1|id2|year_segment),
     ...
     mubin~ (1|id|segment),
     Ubinstd ~(1|id|segment) ,
     nlf(sigmabin~exp(sigmabinstd),loop=FALSE),
     sigmabinstd~(1|id|segment),
...
posterior_predict_sharedpars_exposure <- function(i, prep, ...) {
  ...
  for (j in 1:prep$ndraws){
    N_est[j] = N_int(t,Upois[j]*exposure*1e-7,Ubin[j],mupois[j],sigmapois[j],mubin[j],sigmabin[j])
    #print(N_est[j])#," ",t," ",Upois[j]*exposure*1e-7," ",Ubin[j]," ",mupois[j]," ",sigmapois[j]," ",mubin[j]," ",sigmabin[j])
  }
...
N_pos_plot = pred%>%filter(segment==1,year ==5)%>%
  group_by(.draw)%>%
