...
library(brms)
# writing the formula for the model making use of time and segmentation of the data
formula_exposure <- bf(N_pos_increment | vint(Nprev,N)+vreal(t,tprev,exposure) ~ (1|id2|year_segment),
     mupois~(1|id2|year_segment),
     sigmapois~(1|id2|year_segment),
     mubin~ (1|id2|year_segment),
     Ubin ~(1|id2|year_segment) ,
     sigmabin~(1|id2|year_segment),
...
model_triangles_exposure = brm(
  ...
  prior = mypriors,
  stanvars = stanvar(scode = stan_functions_exposure, block = "functions"),
  data = stan_data_multisegment%>%filter(segment<3), 
  backend = "cmdstan",
...
predN <- predicted_draws(model_triangles_exposure, newdata = stan_data_multisegment%>%filter(segment==1,year ==5))
...
predNpos <- predicted_draws(model_triangles_exposure, newdata = stan_data_multisegment%>%filter(segment==1,year ==5))

N_plot = predN%>%filter(segment==1,year ==5)%>%
   ggplot(aes(x = t, y = .prediction)) +
  stat_lineribbon() +
  scale_fill_brewer() +
 geom_point(data = predN%>%filter(segment==1,year ==5), aes(t, N))

N_pos_plot = predNpos%>%filter(segment==1,year ==5)%>%
  group_by(.draw)%>%
  mutate(N_pos_cum_sample = cumsum(.prediction))%>%ungroup()%>%
   ggplot(aes(x = t, y = N_pos_cum_sample)) +
  stat_lineribbon() +
  scale_fill_brewer() +
 geom_point(data = stan_data_multisegment%>%filter(segment==1,year ==5), aes(t, N_pos_cum))
