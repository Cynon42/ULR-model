...
real sharedpars_exposure_lpmf(int Nposincrement,
 ...
 //print(t,\" \",exp(lnNest));
  // binomial rate expressed as a percentage of the predicted Nprev 
  // (Nest[2,2]-Nest[1,2])/(0.5*(Nest[1,1]+Nest[2,1]))) or (expected Nnegincrement)/(average expected N to avoid inital condition errors)
  return binomial_lpmf(Nnegincrement|Nprev,fmax(1e-100,fmin(0.99,(lnNest[2,2]-lnNest[1,2])/(0.5*(lnNest[1,1]+lnNest[2,1]))))) + poisson_log_lpmf(Nposincrement|log(par[1]) + log_diff_exp(lognormal_lccdf(tprev|par[3],par[4]), lognormal_lccdf(t|par[3],par[4])));
