...
formula_def = bf(N_group_increment | vint(N,Nprev,N_pos_increment,modelflag) ~ lambdapos + log(exposure),
   ...
   nlf(mup~log(80)-2.3*sigmap-exp(mupos)),# Constrained mu for >99% development at t=60
   nlf(lambdapos ~ Upos + logdifffunction(t,tprev,mup,sigmap) ) ,# std incremental poisson model for N+ using modified version of lccdf for syntax and numerical accuracy in the tail.
   nlf(tz~ exp((log(t)-mup)/sigmap)),
   nlf(tprevz~ exp((log(tprev)-mup)/sigmap)),
   nlf(sigman~exp(sigmaneg)),
   #nlf(mun~ mup +(log(80)-2.3*sigman-mup)*inv_logit(muneg-3)),# Constrained mu for >99% development at z-score=5
   nlf(mun~log(80)-2.3*sigman-exp(muneg-3)),# Constrained mu for >99% development at t=80
   nlf(lambdaneg ~ exp( logdifffunction(tz,tprevz,mun,sigman) - exp(Uneg) - logdifffunction(t,0.01,mup,sigmap)) ), # this function will be positive and never greater than 1. derived by suggesting that the N_pos_increment is a poisson rate curve, then divided by N.
   Upos ~ (poly(year,2)|id| segment) + (1|id2|year_segment),
   mupos  + sigmapos ~ (1|id| segment) ,
   Uneg ~ (poly(year,2)|id| segment) + (1|id2|year_segment), 
    muneg  + sigmaneg ~(1|id| segment) ,
  family = freq_def,
  nl=TRUE)
...
mypriors_def <- c(
  prior(normal(5, 2),  nlpar = "Upos"),
  prior(normal(-1, 5),  nlpar = "mupos"),
  prior(normal(0, 2),  nlpar = "sigmapos"),
  prior(normal(0, 2),  nlpar = "Uneg"),
...
for (i in unique(pred_increments$year)){
  ...
  N_plot[[k]] = pred_increments%>%
                ...
                ggplot(aes(x = tprev, y = N_predicted)) +
                stat_lineribbon() +
                scale_fill_brewer() +
                geom_point(data = pred_increments%>%filter(year ==(i)), aes(tprev, N))
  Npos_plot[[k]] = pred_increments%>%
                ...
                stat_lineribbon() +
                scale_fill_brewer() +
                geom_point(data = pred_increments%>%filter(year ==(i)), aes(tprev, N_pos_cum))
  Nneg_plot[[k]] = pred_increments%>%
                ...
                stat_lineribbon() +
                scale_fill_brewer() +
                geom_point(data = pred_increments%>%filter(year ==(i)), aes(tprev, N_pos_cum-N))
