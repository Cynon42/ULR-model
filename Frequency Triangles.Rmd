...
formula_compressed = bf(N_group_increment ~ lambda + log(exposure),
   nlf(lambda ~ U + logdifffunction(t,tprev,log(240)-2.3*exp(sigmadev)-exp(mudev),exp(sigmadev)) ) ,# std incremental poisson model for N+ or N- using modified version of lccdf for syntax and numerical accuracy in the tail. adding development constraint as well
...
# simple formula Binomial only for the negative claims
```{r}
library(brms)
formula_compressed_neg = bf(N_group_increment | trials(N+N_group_increment) ~ lambda ,
   nlf(lambda ~ U + logdifffunction(t,tprev,log(60)-2.3*exp(sigmadev)-exp(mudev),exp(sigmadev)) ) ,# std incremental model for N+ or N- using modified version of lccdf for syntax and numerical accuracy in the tail
   mudev + U + sigmadev~ (1|id2|segment)+(1|id|year_segment),
  family = binomial(),
  nl=TRUE)
...
freq_compressed <- custom_family(
                    "freq_compressed", dpars = c("mu","lambdaneg"), # actually mu is really the lambdapos for the poisson 
...
# Combined formula for Poisson Binomial
```{r}
library(brms)
formula_compressed_combined = bf(N_group_increment | vint(Nprev,N,modelflag)+vreal(t,tprev,exposure) ~ lambdapos + log(exposure),
   nlf(lambdapos ~ Upos + logdifffunction(t,tprev,log(240)-2.3*exp(sigmapos)-exp(mupos),exp(sigmapos)) ) ,# std incremental poisson model for N+ using modified version of lccdf for syntax and numerical accuracy in the tail. Constrained mu for >99% development at t=120
   mupos + Upos + sigmapos+ Uneg~ (1|year_segment),
   nlf(lambdaneg ~ Uneg + logdifffunction(t,tprev,log(240)-2.3*exp(sigmaneg)-exp(muneg),exp(sigmaneg)) ) ,# same model for N- but binomial with additional delay on reports permitted=-==
   muneg  + sigmaneg~ 1,
...
real freq_compressed_lpmf(int Nincrement,
    ...
    return flag==0 ? binomial_lpmf(Nincrement|N+Nincrement,lambdaneg) : poisson_log_lpmf(Nincrement|lambdapos);
...
We identified that the constraint of 99% developed by 5 years was to stringent for some lines and it was relaxed back to 20 years. Much easier to fit the tails on the data now, and much fewer divergences in the longest tailed lines.