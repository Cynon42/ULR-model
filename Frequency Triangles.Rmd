...
formula_compressed = bf(N_group_increment ~ lambda + log(exposure),
   ...
   mudev~ (1|id|model) + (1|id2|segment_model)+(1|year_segment_model),
   U ~(1|id|model) + (1|id2|segment_model)+(1|year_segment_model),
  sigmadev~(1|id|model) + (1|id2|segment_model)+(1|year_segment_model),
  family = poisson(),
  nl=TRUE)
...
model_freq_compressed = brm(
  ...
  data = stan_data_compressed%>%
    #filter(tprev>1,model=="N_pos_increment")%>%
...
this is two orders of magnitude faster than the previous model, but still has divergences.
...
library(ggplot2)
color_scheme_set("darkgray")
mcmc_parcoord(model_freq_compressed, 
              np = nuts_params(model_freq_compressed),transform = function(x) {(x - mean(x)) / sd(x)},
              pars = vars(31:50)
              )+theme(axis.text.x = element_text(angle = 90))
...
freq_compressed <- custom_family(
                    "freq_compressed", dpars = c("mu","mupos","sigmapos","Uneg","muneg","sigmaneg"), # actually mu is really the Ubin for the binomial 
                    links = c("log","identity","log","identity","identity","log"),
                    type = "int",# lpmf vs lpdf
                    vars = c("vint1[n]", # Nprev
                             "vint2[n]", # N
