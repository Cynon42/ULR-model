...
real sharedpars_exposure_lpmf(int Nposincrement,
  ...
  // Set initial values. There should be no claims initially, but we will settle for a very negative number
  lnN0[1] = -1e10; 
  lnN0[2] = -1e10;
  // Set parameters
  // transform development mu such that 99% development at 60 months of development mu = log(x)-2.3*sigma
  ...
  // binomial rate expressed as a percentage of the predicted Nprev 
  // (Nest[2,2]-Nest[1,2])/(0.5*(Nest[1,1]+Nest[2,1]))) or (expected Nnegincrement)/(average expected N to avoid inital condition errors)
  return binomial_lpmf(Nnegincrement|Nprev,fmin(0.99,(exp(lnNest[2,2])-exp(lnNest[1,2]))/(0.5*(exp(lnNest[1,1])+exp(lnNest[2,1]))))) + poisson_log_lpmf(Nposincrement|log(exposure) - log(1e7)+ log(Upois) + log_diff_exp(lognormal_lccdf(tprev|log(60)-2.3*sigmapois-exp(mupois),sigmapois), lognormal_lccdf(t|log(60)-2.3*sigmapois-exp(mupois),sigmapois)));
