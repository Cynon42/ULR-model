...
formula_def = bf(N_group_increment | vint(N,Nprev,N_pos_increment,modelflag) ~ lambdapos + log(exposure),
   nlf(sigmap~exp(sigmapos+1)),
   nlf(mup~log(60)-2.3*sigmap-exp(mupos+1)),# Constrained mu for >99% development at t=60
   ...
   Upos ~ t2(year, by = segment)+(1|year_segment)+(1|id|segment),
   mupos  + sigmapos ~ (poly(year,2)|id|segment),
   nlf(tz~ exp((log(t)-mup)/sigmap)),
   nlf(tprevz~ exp((log(tprev)-mup)/sigmap)),
   nlf(sigman~exp(sigmaneg+1)),
   nlf(mun~5-2.3*sigman-exp(muneg+1)),# Constrained mu for >99% development at z-score=5
   nlf(lambdaneg ~ Uneg + logdifffunction(tz,tprevz,mun,sigman) ) ,# same model for N- but binomial with additional delay on reports permitted
   Uneg + muneg  + sigmaneg ~ (1|id|segment),
  family = freq_def,
  nl=TRUE)
...
mypriors_def <- c(
  prior(normal(5, 2),  nlpar = "Upos"),
  prior(normal(0, 2),  nlpar = "mupos"),
  prior(normal(-1, 1),  nlpar = "sigmapos"),
  prior(normal(-1, 1),  nlpar = "Uneg"),
  prior(normal(3, 3),  nlpar = "muneg"),
  prior(normal(0, 4),  nlpar = "sigmaneg")
  #prior(normal(0, 1),class="Intercept",  dpar = "phineg")
  )
```
...
N_plot = list()
for (i in unique(as.character(pred_increments$year))){
  k=paste0( "year_",i)
  ...
  stat_lineribbon() +
  scale_fill_brewer() +
 geom_point(data = pred_increments%>%filter(year ==as.numeric(i)), aes(t, N))
}
N_plot$year_2+N_plot$year_3
N_plot = pred_increments%>%
  ...
  stat_lineribbon() +
  scale_fill_brewer() +
 geom_point(data = pred_increments%>%filter(year ==selectedyear), aes(t, N))
