...
real[] N_ode_int(real t, 
    ...
    dlnNdt[2] = exp(lnN[1])*U_p*exp(lognormal_lpdf(t|mu_p,sigma_p))/exp(lnN[2]);// gradient of log cumulative # claims dropped back to zero
...
real sharedpars_exposure_lpmf(int Nposincrement,
  ...
  lnN0[1] = log(par[1])+lognormal_lcdf(1.0|par[3],par[4]); 
  lnN0[2] = lnN0[1]-3.0;
  ...
  // Solve ODEs 
  lnNest = integrate_ode_bdf(N_ode_int, //function that will receicve par as input
                         lnN0, // N and N- at t=0
                         1.0, // initial t=1 to avoid initial condition issues
                         times, //array of times at which to find N
                         par,// set of parameters for ode function
                         rep_array(0.0, 0), // not needed real data
                         rep_array(1, 1),// not needed int data
 ...
 //print(t,\" \",exp(lnNest));
  // binomial rate expressed as a percentage of the predicted Nprev 
  // (Nest[2,2]-Nest[1,2])/(0.5*(Nest[1,1]+Nest[2,1]))) or (expected Nnegincrement)/(average expected N to avoid inital condition errors)
  return binomial_lpmf(Nnegincrement|Nprev,fmax(1e-100,fmin(0.99,(exp(lnNest[2,2])-exp(lnNest[1,2]))/(0.5*(exp(lnNest[1,1])+exp(lnNest[2,1])))))) + poisson_log_lpmf(Nposincrement|log(par[1]) + log_diff_exp(lognormal_lccdf(tprev|par[3],par[4]), lognormal_lccdf(t|par[3],par[4])));
...
model_triangles_binom_lnN = brm(
  ...
  data = stan_data_multisegment%>%filter(tprev>2), 
  backend = "cmdstan",
  threads = threading(12),
  control = list(adapt_delta = 0.99),
  max_treedepth=20,
