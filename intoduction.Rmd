...
posterior_predict_binomsharedpars <- function(i, prep, ...) {
  ...
  mupois <- prep_data$resps$Nposincrement$nlpars$mupois$fe$b[i]
  sigmapoisstd <- prep_data$resps$Nposincrement$nlpars$sigmapoisstd$fe$b[i]
  Upoisstd <- prep_data$resps$Nposincrement$nlpars$Upoisstd$fe$b[i]
  Upois = exp(Upoisstd*0.3+4)
  sigmapois = exp(sigmapoisstd+1)
  t <- prep$data$vreal1[i] # vint(Nprev,N)+vreal(t,tprev)
  rpois(prep$ndraws,N_int(t,Upois,Ubin,mupois,sigmapois,mubin,sigmabin))
}


Upoisstd_test <- prep_data$resps$Nposincrement$nlpars$Upoisstd$fe$b
  Upois_test = exp(Upoisstd_test*0.3+4)
library(tidybayes)
library(plotly)
library(ggplot2)
library(patchwork)
pred <- predicted_draws(model, newdata = standata, ndraws= 1000)
