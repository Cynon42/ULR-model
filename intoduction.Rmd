...
fm <-  bf(
   N_pos_increment ~ log(Upois) +log_diff_exp( lognormal_lccdf(tprev,mupois,sigmapois), lognormal_lccdf(t,mupois,sigmapois)) ,# std incremental poisson model for N+ using lccdf for numerical accuracy in the tail
  family = poisson(),
  nl = TRUE) + 
  bf( N_neg_increment | trials(Nprev) ~ (N_neg_int(t,Upois,Ubin,mupois,sigmapois,mubin,sigmabin)-N_neg_int(tprev,Upois,Ubin,mupois,sigmapois,mubin,sigmabin))/N_int(tprev,Upois,Ubin,mupois,sigmapois,mubin,sigmabin),# approximation based on available data
...
mypriors <- c(
  prior(normal(0, 1), nlpar = "Upoisstd"),
  prior(normal(0, 1), nlpar = "Ubinstd"),
  prior(normal(0, 1), nlpar = "sigmapoisstd"),
  prior(normal(0, 1), nlpar = "sigmabinstd"),
  prior(normal(0, 1), nlpar = "mupois"),
  prior(normal(0, 1), nlpar = "mubin"))
```
