...
real[] N_ode_int(real t, 
    ...
    dNdt[1] = U*lognormal_lpdf(t|mu,sigma)-N[1]*U_p*lognormal_lpdf(t|mu_p,sigma_p);// gradient of cumulative # claims >0
    dNdt[2] = N[1]*U_p*lognormal_lpdf(t|mu_p,sigma_p);// gradient of cumulative # claims dropped back to zero
    return dNdt;
}
...
real binomsharedpars_lpmf(int Nnegincrement,
    ...
    if(is_nan(p)==1) print(t);
    return binomial_lpmf(Nnegincrement | N+Nnegincrement,min([0.99, Ubin*exp(log_diff_exp(lognormal_lccdf(tprev|mubin,sigmabin), lognormal_lccdf(t|mubin,sigmabin)))]) );
