...
```{r installcmdstanr, echo = FALSE, eval=FALSE}
...
```{stan, output.var="testmodel", eval=FALSE}
functions {
  vector sho(real t,
             vector y,
             real theta) {
...
formula <-  bf(
  ...
  family = poisson(),
  nl=TRUE)+
  bf(N_neg_increment | trials(Nprev) ~ pbinom , nlf(pbinom~(N_neg_int(t,Upois,Ubin,mupois,sigmapois,mubin,sigmabin)-N_neg_int(tprev,Upois,Ubin,mupois,sigmapois,mubin,sigmabin))/N_int(tprev,Upois,Ubin,mupois,sigmapois,mubin,sigmabin)),# approximation based on available data
...
```
ok. An error. it turns out that there is no easy way to share the poisson model parameters with the binomial model in a way that I can see without modifying the stan code directly...
...
```{stan,eval=FALSE, output.var = "test_code"}
// we solve for #>0 at any point in time t  
vector N_ode(real t,//time
             vector N,// expected # of non-zero claims and expected # dropping to zero at time t. must be a vector so 2d. This is what is being solved for.
             real U,
...
"
```
 this worked but it would be better if the n in the binomial distribution was Nprev + Nposincrement as this is strictly the maximum number of Nnegicrement claims in a period.
...
# Define a family
adding a new statistical family definition in order to pass the needed variables across between the models
...
# modify functions 
and now modify the functions for the family definition and solve ODE
```{r}
stan_functions = "// we solve for #>0 at any point in time t  
vector N_ode(real t,//time
             vector N,// expected # of non-zero claims and expected # dropping to zero at time t. must be a vector so 2d. This is what is being solved for.
...
# visualise with shinystan

now lets visualise the parameters to see how close they were to what we used to generate the data. They won't be a perfect match in any case as the ode method was not used to generate the data, but it should be a good approximation.
...
looking at the parameters, I am sufficiently satisfied to move on and perform some overlay plots.
...
prep_data = prepare_predictions(model) # needed to extract both mv models prep data simultaneously
posterior_predict_binomsharedpars <- function(i, prep, ...) {
  ...
  mubinom <- brms::get_dpar(prep, "mubin", i = i)
  sigmabinom <- brms::get_dpar(prep, "sigmabin", i = i)
  mupois <- prep_data$resps$Nposincrement$nlpars$mupois$fe$b
  sigmapoisstd <- prep_data$resps$Nposincrement$nlpars$sigmapoisstd$fe$b
  Upoisstd <- prep_data$resps$Nposincrement$nlpars$Upoisstd$fe$b
  Upois = exp(Upoisstd*0.3+4)
  sigmapois = exp(sigmapoisstd+1)
  t <- prep$data$vreal1[i] # vint(Nprev,N)+vreal(t,tprev)
  N_est = c()
  for (j in 1:prep$ndraws){
    N_est[j] = N_int(t,Upois[j],Ubinom[j],mupois[j],sigmapois[j],mubinom[j],sigmabinom[j])
  }
...
pred <- predicted_draws(model, newdata = standata)


N_plot = pred%>%filter(.category=="Nnegincrement")%>%
   ggplot(aes(x = t, y = .prediction)) +
  stat_lineribbon() +
  scale_fill_brewer() +
 geom_point(data = pred%>%filter(.category=="Nnegincrement"), aes(t, N))
...
N_plot + N_pos_plot
```
The overlay plots show that the model is following the data, but not fantastically well initially. At this point I realised that my priors did not cover the parameters that I selected very well and, after checking the development pattern I saw that the choices I made when I generated the data weren't great, with much of the development still to be expected after month 50, which is not realistic. So I went back and chose what I thought were better parameters for the synthetic data. So the charts above are the result of improved synthetic data choices where development has halted by month 50 for both the binomial and poisson rate curves.

After an additional bug fix in predicted draws, the charts now look as they should. We can now move on to modelling real world data. So we conclude this chapter of the process as the real world data cannot be shared here.

# Conclusion

It is possible to construct an ODE model that can treat for negative IBNR (Incurred but not reported) claim count data. This treatment using a combination of Poisson and Binomial or just Poisson distributions can reasonably infer expected claim count development patterns when there are two competing processes to contend with: The first process is that of reporting claims and the second, where claims are rejected or fall below deductible and close with no payment.

This type of model is useful for claims reserving and pricing in the insurance sector and can be extended in several key ways using the BRMS/stan modelling architecture.

In the next section we will apply this model to a multi-level model to solve for a standard triangulation exercise.